...
function filterBarMarkup(){
  ...
  return `<div class="filter-bar" id="filterBar"><button type="button" class="filter-drawer-close" id="filterDrawerClose" aria-label="Close filters">${ico('x')}</button><div class="fb-group" style="flex-wrap:wrap">
...
function bindFilterEvents(){
  ...
  const drawerClose=$('#filterDrawerClose');if(drawerClose)drawerClose.onclick=()=>{bar.classList.remove('open');backdrop.classList.remove('open')};
}
...
<div class="detail-layout">
  <div class="detail-main">
    ...
    <div class="panel"><h2>Customer Reviews</h2>${ratingRowHtml(game,item.id)||'<p style="color:var(--muted);font-size:13.5px;margin-bottom:14px">No reviews yet for this service — be the first to complete an order and leave one.</p>'}
      ...
      <div class="review-form"><h3 style="font-size:14.5px;margin-bottom:6px">Leave a review</h3><p style="color:var(--muted);font-size:12.5px;margin-bottom:10px">Only customers with a completed order for this service can review it. Enter your order reference (e.g. K3-XXXXXXXX) to verify.</p>
        ...
        <div class="stars-input" id="rvStars" role="radiogroup" aria-label="Star rating">${[1,2,3,4,5].map(n=>`<button type="button" data-star="${n}" aria-label="${n} star${n===1?'':'s'}" aria-pressed="false">${starIcon(false)}</button>`).join('')}</div>
  ...
  starsWrap.addEventListener('click',e=>{const b=e.target.closest('[data-star]');if(!b)return;chosenStars=Number(b.dataset.star);$$('button',starsWrap).forEach(btn=>{const n=Number(btn.dataset.star);const active=n<=chosenStars;btn.classList.toggle('active',active);btn.setAttribute('aria-pressed',String(active));btn.innerHTML=starIcon(active)})});
...
async function renderRobloxLogin(){
  ...
  document.body.className='';
  const connected=Boolean(state.session.roblox);
  // Someone who already connected Roblox on an earlier order shouldn't be sold the
  // OAuth pitch a second time — that's the "why is it asking again" problem. If we
  // already have their identity, this screen skips straight to reviewing the order
  // and only the account-recap + "change account" path stays visible.
  const step2Class=connected?'done':'current', step3Class=connected?'current':'';
  const heroCopy=connected
    ? `<p class="kicker"><span class="dot"></span>Step 2 of 3 — done</p><h1>You're Already Connected</h1><p>This order will be fulfilled on the Roblox account below. Confirm it's the right one, then continue to review.</p>`
    : `<p class="kicker"><span class="dot"></span>Secure checkout</p><h1>Connect Your Roblox Identity</h1><p>Before you can confirm an order, sign in with Roblox directly. This order will be fulfilled for whichever account you connect here.</p><ul class="trust" style="margin:16px 0"><li>We receive your Roblox username, display name, user ID and public profile details — never your password</li><li>Roblox doesn't share an email address with us, so this identity is what appears on your order and receipt</li><li>Nothing is shared until you complete Roblox's own sign-in page</li></ul>`;
  document.body.innerHTML=`${header()}<main class="auth-page" id="main"><section class="auth-card panel" style="width:min(560px,100%)">
<div class="progress-track"><div class="progress-step done"><span class="ps-dot">${ico('check')}</span><span class="ps-label">Cart</span></div><div class="progress-line"></div><div class="progress-step ${step2Class}"><span class="ps-dot">${connected?ico('check'):'2'}</span><span class="ps-label">Roblox</span></div><div class="progress-line"></div><div class="progress-step ${step3Class}"><span class="ps-dot">3</span><span class="ps-label">Review</span></div></div>
${heroCopy}<div id="identityArea"></div><div class="error" id="error" role="alert"></div><button class="btn blue" style="width:100%" id="robloxAction">${connected?'Continue to review':'Connect with Roblox'}</button></section></main>${footer()}<div class="modal" id="modal"></div>`;nav();
  const action=$('#robloxAction');
  if(connected){$('#identityArea').innerHTML=`<div class="identity"><b>${esc(state.session.roblox.displayName)}</b>@${esc(state.session.roblox.username)}<br><span>Not the right account? Use “Change account” below — reconnecting isn't available once you're reviewing the order.</span>${robloxDetailGrid(state.session.roblox)}</div><button type="button" class="btn ghost" id="changeAccount" style="width:100%;margin-bottom:12px">Change account</button>`;action.onclick=()=>reviewOrder(saved);
...
function reviewOrder(saved){
  ...
  const closeModal=()=>{modal.classList.remove('show');document.removeEventListener('keydown',onKey);if(opener&&opener.focus)opener.focus()};
  const onKey=e=>{if(e.key==='Escape')closeModal()};
  document.addEventListener('keydown',onKey);
  modal.classList.add('show');$('.modal-close',modal).onclick=closeModal;
...
async function renderAccountOrders(){
  ...
  panel.innerHTML=`<div class="local-note">${ico('info')}<span>Orders placed while your Roblox account was connected are tied to that account, not this device — sign in with the same Roblox account anywhere and they'll show up here with live status. Rate requests (sent before you connect Roblox) don't have an account to sync to, so those stay local to this browser only.</span></div><div id="ordersBody"><p style="color:var(--muted);font-size:13px">Loading your orders…</p></div>`;
  let serverOrders=[];
...
function renderSupport(){
  ...
  <div class="policy-block" id="contact"><h2>Contact support</h2><p>Submit a support or refund request below, referencing your order or rate-request ID (format <code>K3-XXXXXXXX</code> or <code>Q-XXXXXXXX</code>). You'll get a request reference back and can check its status any time using the lookup underneath. You can also review a local copy of your recent orders from <a href="/account.html?tab=orders" style="color:var(--accent2)">Your account</a>.</p>
    ...
    <div class="panel" style="padding:22px;margin-top:16px">
      ...
      <div class="ticket-form ticket-form-2col">
